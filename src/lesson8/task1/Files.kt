...
fun alignFileByWidth(inputName: String, outputName: String) {
    ...
    for (i in 0 until input.size) {
        val idealElement = input[i].replace(Regex(" {2,}"), " ").trim()
        if (idealElement.length > maxLine) {
            maxLine = idealElement.length
        }
        newInput.add(idealElement)
    }
    for (element in newInput) {
        ...
        val wordLine = element.split(" ")
        val spaceNum = maxLine - (element.length - wordLine.size + 1)
        if (element.replace(Regex(" *"), "") != "") {
            if (wordLine.size == 1){
                finalString +=wordLine[0]
            }
            if (wordLine.size > 1){
                val space = space(spaceNum, wordLine.size - 1)
                finalString += wordLine[0]
                for (i in 0 until space.size){
                    finalString = finalString + space[i] + wordLine[i + 1]
                }
            }
    ...
    return File(outputName).writeText(finalString)
}
